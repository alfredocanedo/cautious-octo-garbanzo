...
if __name__ == "__main__":
    print("=" * 60)
    print("AI FINGERPRINT REMOVER - TEST RUN")
    print("=" * 60)
    
    # Create sample audio data (2 seconds at 22050 Hz)
    sample_rate = 22050
    duration = 2.0
    num_samples = int(sample_rate * duration)
    
    # Generate synthetic AI-like audio (sine wave with perfect properties)
    t = np.linspace(0, duration, num_samples)
    audio_data = 0.3 * np.sin(2 * np.pi * 440 * t)  # 440 Hz sine wave
    
    print(f"\n✓ Generated synthetic audio: {duration}s at {sample_rate}Hz")
    print(f"✓ Audio shape: {audio_data.shape}")
    print(f"✓ Audio range: [{audio_data.min():.4f}, {audio_data.max():.4f}]")
    
    # Initialize remover in normal mode
    print("\n--- Running Fingerprint Removal (Normal Mode) ---")
    remover = FingerprintRemover(paranoid_mode=False)
    result = remover.remove_fingerprints(audio_data, sample_rate)
    
    print(f"\n✓ Fingerprints Detected: {len(result['fingerprints_detected'])}")
    if result['fingerprints_detected']:
        for fp in result['fingerprints_detected']:
            print(f"  - {fp['type']}: {fp['value']:.4f} (target: {fp['target_range']})")
    
    print(f"\n✓ Removal Methods Applied:")
    for method in result['removal_methods']:
        print(f"  - {method}")
    ...
    # Test paranoid mode
    print("\n--- Running Fingerprint Removal (Paranoid Mode) ---")
    remover_paranoid = FingerprintRemover(paranoid_mode=True)
    result_paranoid = remover_paranoid.remove_fingerprints(audio_data, sample_rate)
    
    print(f"\n✓ Removal Methods Applied (Paranoid):")
    for method in result_paranoid['removal_methods']:
        print(f"  - {method}")
    
    # Test machine perfection removal
    print("\n--- Testing Machine Perfection Pattern Removal ---")
    cleaned_perfect = remover.remove_machine_perfection_patterns(audio_data)
    print(f"✓ Machine perfection patterns removed")
    print(f"✓ Cleaned audio shape: {cleaned_perfect.shape}")
